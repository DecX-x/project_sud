...
class BinDetailScreen extends ConsumerStatefulWidget {
  final String binId;

  const BinDetailScreen({super.key, required this.binId});

  @override
  ConsumerState<BinDetailScreen> createState() => _BinDetailScreenState();
}

class _BinDetailScreenState extends ConsumerState<BinDetailScreen> {
  Timer? _refreshTimer;
  ...
  void initState() {
    ...
    _refreshTimer = Timer.periodic(const Duration(seconds: 3), (_) {
      ref.invalidate(binByIdProvider(widget.binId));
    });
  }
  ...
  Widget build(BuildContext context) {
    final binAsync = ref.watch(binByIdProvider(widget.binId));
    ...
    return Scaffold(
      ...
      body: binAsync.when(
        data: (bin) {
          ...
          return _buildContent(context, bin, isDark);
        },
  ...
  Widget _buildContent(BuildContext context, BinModel bin, bool isDark) {
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildInfoRow(
                    context,
                    'Last Updated',
                    DateFormat('MMM dd, yyyy HH:mm').format(bin.lastUpdated),
                    Colors.blue,
                  ),
                ],
              ),
